...
import type { Config, RunMetrics, HttpTimingResult, LighthouseResult } from '../types/index.js';
...
export class Protocol {
  ...
  config(config: Config): void {
    ...
    if (config.network?.throttle) {
      ...
      if (profile) {
        this.lines.push(`│   Download:      ${formatBytes(profile.downloadThroughput)}/s`);
        this.lines.push(`│   Upload:        ${formatBytes(profile.uploadThroughput)}/s`);
    ...
    this.lines.push(`│ HTTP-Timing:     ${config.httpTiming?.enabled ? 'enabled' : 'disabled'}`);
    if (config.httpTiming?.enabled) {
      this.lines.push(`│   Runs:          ${config.httpTiming.runs}`);
      this.lines.push(`│   Wait Between:  ${config.httpTiming.waitBetweenRuns}ms`);
    }
    this.lines.push('│');

    this.lines.push(`│ Blocklists:`);
    this.lines.push(`│   Third-Party (${config.blocklists.thirdParty.length}):`);
    for (const p of config.blocklists.thirdParty) {
      this.lines.push(`│     ${p}`);
    }
    this.lines.push(`│   Tracking-Only (${config.blocklists.trackingOnly.length}):`);
  ...
  domainStart(name: string, index: number, total: number): void {
    this.lines.push(`┌─ DOMAIN: ${name} (${index}/${total}) ─────────────────────────────────`);
  }
  ...
  playwrightColdResult(scenario: string, metrics: RunMetrics, durationMs: number): void {
    this.lines.push(line(1, `  Duration: ${formatMs(durationMs)}`));
    this.lines.push(line(1, `  HTML: ${formatBytes(metrics.html.transferSize)} transfer | SSR: ${metrics.html.isSSR ? 'yes' : 'no'}`));
    this.lines.push(line(1, `  JS:   ${formatBytes(metrics.javascript.transferSize)} transfer | ${Math.round(metrics.javascript.unusedRatio * 100)}% unused | ${metrics.javascript.chunks.total} chunks`));
    this.lines.push(line(1, `  CSS:  ${formatBytes(metrics.css.transferSize)} transfer | ${Math.round(metrics.css.unusedRatio * 100)}% unused`));
    this.lines.push(line(1, `  3P:   ${formatBytes(metrics.thirdParty.totalTransferSize)} | ${metrics.thirdParty.totalRequests} requests`));
    this.lines.push(line(1, ''));
  }

  playwrightWarmStart(scenario: string, run: number, total: number): void {
    this.lines.push(line(1, `[${ts()}] PLAYWRIGHT › ${scenario} › warm run ${run}/${total}`));
  }

  playwrightWarmResult(scenario: string, run: number, metrics: RunMetrics, durationMs: number): void {
    this.lines.push(line(1, `  Duration: ${formatMs(durationMs)}`));
    this.lines.push(line(1, `  JS: ${formatBytes(metrics.javascript.transferSize)} | 3P: ${formatBytes(metrics.thirdParty.totalTransferSize)}`));
    this.lines.push(line(1, ''));
  }

  lighthouseStart(scenario: string, runType: 'cold' | 'warm'): void {
    this.lines.push(line(1, `[${ts()}] LIGHTHOUSE › ${scenario} › ${runType} run`));
  }

  lighthouseResult(scenario: string, runType: 'cold' | 'warm', result: LighthouseResult): void {
    this.lines.push(line(1, `  Performance Score: ${(result.performanceScore * 100).toFixed(0)}/100`));
  ...
  httpTimingResult(result: HttpTimingResult): void {
    this.lines.push(line(1, `  TTFB    p50: ${formatMs(result.ttfb.p50)} | p95: ${formatMs(result.ttfb.p95)} | p99: ${formatMs(result.ttfb.p99)}`));
    this.lines.push(line(1, `  Total   p50: ${formatMs(result.total.p50)} | p95: ${formatMs(result.total.p95)} | p99: ${formatMs(result.total.p99)}`));
    this.lines.push(line(1, `  Status: ${result.statusCode} | Protocol: ${result.protocol} | Size: ${formatBytes(result.responseSize)}`));
    if (result.server) {
